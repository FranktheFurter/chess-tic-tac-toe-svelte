...
export function isValidMove(
    ...
    switch (piece.type) {
        case 'pawn':
            ...
            const validForward = to.row === from.row + direction && to.col === from.col && !board[to.row][to.col];
            const validCapture = to.row === from.row + direction && Math.abs(to.col - from.col) === 1 && board[to.row][to.col];
...
export function checkWinner(board: (Piece | null)[][]): 'white' | 'black' | null {
    // Check rows
    for (let row = 0; row < 3; row++) {
        if (board[row][0] && 
            board[row][1] && 
